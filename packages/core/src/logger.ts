...
export const logger = {
   ...
   */
  banner(version: string, env: string, mode: string, port = PORTS.CLI) {
    ...
    console.log(`  ${colors.zinc}➜${colors.reset}  ${colors.bright}Local Host: ${colors.reset}  ${colors.info}${colors.underscore}http://localhost:${port}${colors.reset}`);
    console.log("");
  },
  ...
  divider() {
    const cols = typeof process !== 'undefined' && process.stdout ? (process.stdout.columns || 40) : 40;
    console.log(`${colors.gray}${"─".repeat(cols)}${colors.reset}`);
  }
};
