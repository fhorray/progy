...
6.  **CLI:** Initializes a *fresh* Git repository in the student's folder.
...
This command is for instructors. It takes the *pre-built* `.progy` artifact and uploads it.
...
2.  **Validation:**
    *   Read `course.json` *from inside* the `.progy` file (or assume the one in CWD matches).
...
5.  **Portfolio Setup (The Git Logic):**
    ...
    *   **Step C:** (Optional) Ask user: *"Do you want to connect this to GitHub?"*
        *   If YES:
    ...
    *   **Result:** The student has a clean git history starting from the moment they downloaded the course. They are *not* attached to the instructor's history.
...
export async function setupStudentPortfolio(cwd: string, courseName: string, token: string) {
  ...
  const shouldConnect = await confirm({ message: "Create a private GitHub repository for this course?" });
