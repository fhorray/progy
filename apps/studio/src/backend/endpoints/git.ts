import { GitUtils, getCourseConfig } from "@progy/core";
...
const getStatus = async () => {
  ...
  const changes = lines
    ...
    .filter((change: { code: string; file: string }) => {
      ...
      const isHidden = filePath.split("/").some((part: string) => part.startsWith(".") && part !== ".");
  ...
  try {
    ...
    try {
      ...
      const filesToAdd = statusRes.stdout.split("\n")
        ...
        .map((l: string) => l.substring(2).trim().replace(/\\/g, "/")) // Robust parsing
        .filter((f: string) => {
          ...
          const isHidden = f.split("/").some((part: string) => part.startsWith(".") && part !== ".");
