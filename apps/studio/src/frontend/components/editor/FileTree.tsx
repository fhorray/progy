...
const getFileIcon = (name: string, isOpen: boolean = false) => {
  ...
  if (name.endsWith('.sh') || name === 'Dockerfile')
    return <Terminal size={14} className="text-green-500" />;

  // Default Dir/File
  if (!name.includes('.')) {
    // Rough heuristic for dir if passed only name, but FileNode knows type
    return isOpen ? (
...
function FileTreeNode({
  ...
  const isActive =
    ...
    (isModule && activePath === `${node.path}/info.toml`);
    ...
    <div>
      ...
      {isOpen && (
        <div className="border-l border-zinc-800/50 ml-3">
          {isLoading ? (
            <div className="py-1 pl-6 text-xs text-zinc-600 italic">
              Loading...
            </div>
          ) : (
            children.map((child) => (
...
export function FileTree() {
  ...
  const filteredFiles = rootFiles.filter((f) => {
    ...
    if (name === 'course.json' || name === '.progy' || name === 'info.toml')
      return false;
    ...
    <div className="h-full flex flex-col overflow-hidden bg-zinc-950">
      ...
      <div className="flex items-center gap-1 px-3 py-2 border-b border-zinc-800/80 bg-zinc-900/20">
        ...
        <button
          ...
          className="p-1.5 rounded hover:bg-zinc-800 text-zinc-500 hover:text-purple-400 transition-colors"
          title="New Quiz"
        >
        ...
        {filteredFiles.length === 0 ? (
          <div className="px-4 py-8 text-center">
            ...
            <p className="text-xs text-zinc-600">
              {rootFiles.length === 0 ? 'Loading...' : 'No files found'}
            </p>
          </div>
        ) : (
