...
function detectFileType(path: string): EditorTab['type'] {
  ...
  if (['png', 'jpg', 'jpeg', 'gif', 'svg', 'webp'].includes(ext || '')) return 'code'; // Treat images as special or just prevent JSON parsing
  return 'code';
}
  ...
  if (!tabs[file.path]) {
    try {
      ...
      if (isBinaryFile(file.path)) {
        // Don't open binary files in the editor for now, or handle them as virtual tabs
        console.log('[EditorStore] Skipping binary file open:', file.path);
        return;
      }
