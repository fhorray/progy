        ...
        <div className="space-y-3">
          ...
          <div>
            ...
            <p className="text-[11px] text-zinc-600 mt-1">
              Shown to students at the start of this module
            </p>
          </div>
  ...
  const handleRename = async () => {
    ...
    try {
      const res = await fetch(
        `/instructor/fs?path=${encodeURIComponent(path)}`,
        {
          method: 'PATCH',
          headers: { 'Content-Type': 'application/json' },
...
export function NewLessonDialog({
  ...
  return (
    <DialogBackdrop onClose={onClose}>
      <div className="p-5">
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-sm font-bold text-zinc-100">New Lesson</h3>
          <button
            onClick={onClose}
            className="p-1 rounded hover:bg-zinc-800 text-zinc-500"
          >
        ...
        {isLoading ? (
          ...
          <div className="space-y-3">
            ...
            <div>
              ...
              />
              <p className="text-[11px] text-zinc-600 mt-1">
                This title goes into info.toml and the student navigation
              </p>
            </div>

            <div>
              <label className="block text-xs font-medium text-zinc-400 mb-1.5">
                Starter File Extension
              </label>
              <input
                type="text"
                value={fileExtension}
              ...
              />
              <p className="text-[11px] text-zinc-600 mt-1">
                Creates {fileExtension === 'rs' ? 'exercise' : 'main'}.
                {fileExtension || 'ext'} + README.md
              </p>
            </div>
        ...
        <div className="flex justify-end gap-2 mt-5">
          ...
          <button
            onClick={handleCreate}
            disabled={
              isCreating || !title.trim() || !modulePath || modules.length === 0
            }
...
export function NewQuizDialog({
  ...
  return (
    <DialogBackdrop onClose={onClose}>
      <div className="p-5">
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-sm font-bold text-zinc-100">New Quiz</h3>
          <button
            onClick={onClose}
            className="p-1 rounded hover:bg-zinc-800 text-zinc-500"
          >
          ...
          <div className="space-y-3">
            ...
            <div>
              ...
              <input
                ...
                value={title}
                onChange={(e) => setTitle(e.target.value)}
                placeholder="e.g. Logic Challenge"
                className="w-full bg-zinc-800/80 border border-zinc-700 rounded-lg px-3 py-2 text-sm text-zinc-100 placeholder-zinc-600 outline-none focus:ring-2 focus:ring-purple-500/50 focus:border-purple-500/50"
                autoFocus
                onKeyDown={(e) => e.key === 'Enter' && handleCreate()}
              />
              <p className="text-[11px] text-zinc-600 mt-1">
                This quiz will contain multiple-choice questions
              </p>
            </div>
        ...
        <div className="flex justify-end gap-2 mt-5">
          ...
          <button
            ...
            className="px-4 py-1.5 bg-purple-600 hover:bg-purple-500 text-white text-xs font-medium rounded-lg transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
          >
      ...
      <div className="p-5">
        ...
        {isProtected ? (
          <p className="text-sm text-amber-400 bg-amber-500/10 px-3 py-3 rounded-lg mb-4">
            ⚠️ <strong>{filename}</strong> is a protected file and cannot be
            deleted.
          </p>
        ) : (
          <p className="text-sm text-zinc-400 mb-4">
            Are you sure you want to delete{' '}
            <strong className="text-zinc-200">{filename}</strong>? This action
            cannot be undone.
          </p>
        )}

        {error && (
          <p className="text-xs text-red-400 bg-red-500/10 px-3 py-2 rounded-lg mb-4">
            {error}
          </p>
        )}
